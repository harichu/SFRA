 ...
 */
function updateTotals(totals, shippingmethod) {
    $(".shipping-total-cost").text(totals.totalShippingCost);
    $(".tax-total").text(totals.totalTax);
    $(".sub-total").text(totals.subTotal);
    $(".sub-total-club").text(totals.clubTotal);
    $(".grand-total-sum").text(totals.grandTotal);
    $(".club-grand-total-sum").text(totals.clubGrandTotal);
    $(".shipping-item .shipping-method-name").text(shippingmethod[0].selectedShippingMethod.displayName);
...
function updateOrderProductSummaryInformation(order) {
    ...
    $(".summary-card-body").html($productSummary.html());
}
