...
module.exports = function (object, quantity, minOrderQuantity, availabilityModel) {
    ...
    /* A zone may have an invalid inventory set (inventory was deleted, or never existed).
     * In that case, the availability model defaults to the auxiliary inventory, which is
     * configured to use "default in stock", making all products available. We check here
     * if that is the case, and if so mark all products as unavailable instead.
     */
    var isInventoryValid = !empty(storeHelper.getZoneInventory());

    availabilityModel = productHelper.getAvailabilityModel(object.id);
    ...
    Object.defineProperty(object, "available", {
        enumerable: true,
        value: (function () {
            var invRec = productHelper.getAvailabilityModelByZone(object.id);
            if (!invRec || !isInventoryValid) {
                return false;
            } else {
                return availabilityModel.isOrderable(parseFloat(quantity) || minOrderQuantity);
            }
        }()) 
    });
};
