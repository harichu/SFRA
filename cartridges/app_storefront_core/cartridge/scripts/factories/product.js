...
module.exports = {
    get: function (params) {
        ...
        if (apiProduct === null) {
            return product;
        } else if (apiProduct.variants.length > 0 && params.variables != null) {
            var variantProduct = productHelper.getVariationProduct(apiProduct.variants, params.variables); 
            if (variantProduct != null) {
                apiProduct = variantProduct;
            }
        } else if (apiProduct.variants.length > 0 && empty(apiProduct.variationModel.selectedVariant)) {
            if (!empty(apiProduct.variationModel.defaultVariant)) {
                apiProduct = ProductMgr.getProduct(apiProduct.variationModel.defaultVariant.ID);
            } else {
                apiProduct = apiProduct.variationModel.variants[0];
            }   
        }
