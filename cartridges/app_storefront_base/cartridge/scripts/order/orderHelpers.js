...
function getOrders(currentCustomer, querystring, locale) {
    ...
    while (customerOrders.hasNext()) {
        ...
        } else if ((querystring.orderFilter && YEAR_AGO < orderTime && querystring.orderFilter === "12") || SIX_MONTHS_AGO < orderTime){
            orderModel = new OrderModel(customerOrder, { config: config, countryCode: currentLocale.country });
            orders.push(orderModel);
        }
    }
