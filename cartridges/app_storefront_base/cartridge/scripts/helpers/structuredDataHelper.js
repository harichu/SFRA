...
function getProductSchema(product) {
    ...
    if (product.images && product.images.pdplarge) {
        if (!schema.image) {
            schema.image = [];
        }
        product.images.pdplarge.forEach(function (image) {
            schema.image.push(image.absURL || image.url);
        });
    }
    if (product.price) {
        schema.offers = {
        ...
        if (product.price.type === "range") {
            ...
            if (product.price.sales && product.price.sales.decimalPrice != null) {
                schema.offers.priceCurrency = product.price.sales.currency;
                schema.offers.price = product.price.sales.decimalPrice;
            } else if (product.price.list && product.price.list.decimalPrice != null) {
