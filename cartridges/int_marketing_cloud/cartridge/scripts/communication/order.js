...
function confirmation(promise, data) {
    ...
    var order = data.params.Order;
    var orderXml = helpers.stripXmlNS( data.params.Order.getOrderExportXML(null, null) );
    var allProducts = order.allProductLineItems.iterator();

    while (allProducts.hasNext()) {
        var productLine  = allProducts.next();
        var id       = productLine.product.ID;
        var product  = ProductMgr.getProduct(id);
        var idTag    = "<product-id>" + id + "</product-id>";     
        var brandTag = "\r\n<brand>" + encodeURIComponent(product.brand) + "</brand>\r\n";
        var imgUrlTag = "\r\n<imgUrl>" + encodeURIComponent(product.getImages("large")[0].httpsURL) + "</imgUrl>\r\n";
        var regexp   = new RegExp(idTag, "g");
        orderXml     = orderXml.replace(regexp, idTag + brandTag + imgUrlTag);
    }

    data.orderAsXML = orderXml;
