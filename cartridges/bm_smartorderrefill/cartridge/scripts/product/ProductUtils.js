...
ProductUtils.getAvailability = function (item, quantity) {
    ...
    var avm = productHelper.getAvailabilityModel(item);
...
/**
 * Gets Product "original" DIS Image
 *
 * @param {dw.catalog.Product} product
 * @param {string} imageType - String with one of the values configured in catalog image settings.
 * @param {object} jsonConfig - A json config {scaleWidth: XX, scaleHeight: YY}
 * @param {number} index - Optional. In case product has more than one image, return the first or the one in this parameter
 * @returns {string} - Resulting DIS Url
 */
ProductUtils.getImageUrl = function (product, imageType, jsonConfig, index) {
    ...
    jsonConfig.scaleMode = jsonConfig.scaleMode || 'fit';
    let image = product.getImage(imageType, index);
    return !empty(image) ? image.getImageURL(jsonConfig) : null;
};
 ...
 *
 * @param {dw.catalog.Product} product
 * @param {string} imageType - One of the valid image types configured on catalog.
 * @param {number} index - Optional. In case product has more than one image, return the first or the one in this parameter
 * @returns {string} - Url to a DIS image
 */
ProductUtils.getImagePresetUrl = function (product, imageType, index) {
    try {
        let configsData = ProductUtils.getImagePreset(imageType);
        if (empty(configsData)) {
            return null;
        }
        return ProductUtils.getImageUrl(product, imageType, configsData, index);
    } catch(e) {
        dw.system.Logger.error('ERROR in ProductUtils.js: {0}', e);
        return null;
    }
};

/**
 * Gets JSON Image preset config
 *
 * @param {string} imageType - One of the valid image types configured on catalog.
 * @returns {object} - JSON config
 */
ProductUtils.getImagePreset = function(imageType) {
    let configsData = null;

    try {
        configsData           = JSON.parse('{"scaleWidth": 120, "scaleHeight": 120}');
        configsData.scaleMode = configsData.scaleMode || 'fit';
    }
    catch (e) {
        dw.system.Logger.error('ERROR while getting image preset confgis ProductUtils.js: {0}', e);
    } finally {
        return configsData;
    }
};
