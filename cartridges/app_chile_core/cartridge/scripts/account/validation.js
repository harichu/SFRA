...
function validateFilledFields(form, isEditing) {
    var errors = {};
    if (!validation.validateRut(form.rut)) {
        errors.rut = Resource.msg("error.rut.invalid", "forms", null);
    } else if (!isEditing && validation.rutAlreadyRegistered(form.rut)) {
        errors.rut = Resource.msg("error.rut.registered", "forms", null);
    }
...
function validate(form, isEditing) {
    var fields = ["rut", "fullName", "phone", "gender", "birth", "dwfrm_profile_customer_email", "password", "passwordConfirm", "termsAndConditions"];
